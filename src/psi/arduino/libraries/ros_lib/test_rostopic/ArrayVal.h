#ifndef _ROS_test_rostopic_ArrayVal_h
#define _ROS_test_rostopic_ArrayVal_h
...
namespace test_rostopic
{
  ...
  class ArrayVal : public ros::Msg
  {
    public:
      ...
      typedef test_rostopic::Val _vals_type;
      _vals_type st_vals;
      _vals_type * vals;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(vals_lengthT > vals_length)
        this->vals = (test_rostopic::Val*)realloc(this->vals, vals_lengthT * sizeof(test_rostopic::Val));
      vals_length = vals_lengthT;
      for( uint32_t i = 0; i < vals_length; i++){
      offset += this->st_vals.deserialize(inbuffer + offset);
        memcpy( &(this->vals[i]), &(this->st_vals), sizeof(test_rostopic::Val));
      }
     return offset;
    }

    const char * getType(){ return "test_rostopic/ArrayVal"; };
